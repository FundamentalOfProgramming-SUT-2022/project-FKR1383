...
bool is_cut = false , is_find = false , is_replace = false , is_arman = false , is_arman2 = false , one_word = false;
char pathes[maximum_size_of_input] , pathes2[maximum_size_of_input] , pathes3[maximum_size_of_input] , cb[maximum_size_of_input];
...
void find_grep(FILE * , char * , int , int , int * , char *); // this function use for printing a line (by another function) that has some text (pattern)
void print_the_line(char * , int , int * , int , char *); // this function use for printing a line that starts after last_enter and push the index to next enter
...
bool is_directory(char *); // this function checks that the path shows directory or regular file
...
void pastestr(char *command) {
    char *paste = (char *)calloc(maximum_size_of_input , sizeof(char));
    strcpy(paste , cb);
...
void grep(char *command) {
    ...
    char *help = (char *) calloc(maximum_size_of_input , sizeof(char));
    char *result = (char *)calloc(maximum_size_of_input , sizeof(char));
    int skip;
    strcpy(pattern , "\0");
    strcpy(help , "\0");
    ...
    while (newcommand != NULL) {
        ...
        i++;
        int before = lines;
        find_grep(file , pattern , i , mode , &lines , result);
        if (before != lines && mode == 2) {
            strcat(result , pathes);
            strcat(result , "\n");
        }
        fclose(file);
        file = NULL;
    ...
    if (!is_arman) {
        ...
        strcpy(arman_string , result);
        arman(save);
    }
    is_arman = false;
}

void find_grep(FILE *file , char *pattern , int id , int mode , int *lines , char *result) {
    ...
    for (int i = 0; i <= strlen(text); i++) {
        if (j == strlen(pattern)) {
            if (mode == 0) {
                strcat(result , pathes);
                strcat(result , ": ");
                print_the_line(text, last_enter, &i , mode , result);
            } if (mode == 1 || mode == 2)
                print_the_line(text, last_enter, &i , mode , result);
            (*lines)++;
            j = 0;
...
void print_the_line(char *text , int last_enter , int *ind , int mode , char *result) {
    int i = last_enter;
    if (last_enter == -1)
        i = 0;
    while (text[i] == '\n' || text[i] == '\0')
...
void find(char *command , char *rep) {
            ...
            if (string[0] == 127) {
                ...
                if (string[checker] == 127) {
                    ...
                    *(first_pos + counter) = l+1;
                    ...
                    for (int h = i-1; h != -1 && all_text[h] != ' ' && all_text[h] != '\n'; h--) {
                        if (string[checker] == 127)
                            break;
